...
def get_params():
    ...
    parser.add_argument(
        '--Qmodel_path', default='/media/ders/mazhiming/SP_CAM_code/SPCAM/models_ckpt/Q_model_final.pth', type=str)  #
    parser.add_argument(
        '--Cmodel_path', default='log/eps_base_pcm_spsal_Qtrans15/checkpoint_best.pth', type=str)  #
    ...
    def get_cam(self, images, ids, Qs):
        with torch.no_grad():
            ...
            for s, q in zip(self.scale_list, Qs):
                ...
                if (self.SP_CAM):
                    pred, logits, _ = self.C_model(scaled_images)
                else:
                    _, logits, _ = self.C_model(scaled_images)

                # cam_sp=self.Qtransfer(logits,_,Qs[0])
                # pred = F.softmax(cam_sp, dim=1)
                pred = F.softmax(logits, dim=1)
                cam_list.append(torch.roll(pred, 1, dims=1))
...
if __name__ == "__main__":
    ...
    evaluatorA = evaluator(dataset='voc12', domain=args.domain, muti_scale=True, SP_CAM=args.sp_cam,
                           save_np_path=_savenpy_path, savepng_path=_savepng_path, refine_list=[0, 20, 30, 40], th_list=[0.2, 0.25, 0.3])
    # evaluatorA =evaluator(args.dataset,domain=args.domain ,SP_CAM=False,refine_list=[0,20])
